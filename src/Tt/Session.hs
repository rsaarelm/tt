...
sessions _ [] = []
sessions zt (In t1 project _:Out t2 _:ts) =
  Session project (zonedTimeToUTC t1) (zonedTimeToUTC t2) : sessions zt ts
-- Started clock that hasn't closed yet, this is fine, but we can't do the span without a current time.
sessions zt [In t1 project _] =
  [Session project (zonedTimeToUTC t1) (zonedTimeToUTC zt)]
-- TODO: Proper warnings from bad data
sessions zt (Out{}:ts) = trace "Unmatched clock out" sessions zt ts
sessions zt (In{}:In t2 p t:ts) =
  trace "Unmatched clock in" sessions zt (In t2 p t : ts)

sessionLength :: Session -> NominalDiffTime
sessionLength Session { sessionStart = start, sessionEnd = end } =
  end `diffUTCTime` start

-- | Time span for the day of the given zoned time stamp.
daySpan :: ZonedTime -> (UTCTime, UTCTime)
daySpan zt = (zonedTimeToUTC start, zonedTimeToUTC end)
 where
  start = zt { zonedTimeToLocalTime = startOfDay (zonedTimeToLocalTime zt) }
  end   = zt { zonedTimeToLocalTime = endOfDay (zonedTimeToLocalTime zt) }
  startOfDay t = t { localTimeOfDay = midnight }
  endOfDay t =
    LocalTime {localTimeOfDay = midnight, localDay = addDays 1 (localDay t)}

-- | Time span for the month of the given zoned time stamp.
monthSpan :: ZonedTime -> (UTCTime, UTCTime)
monthSpan zt = (zonedTimeToUTC start, zonedTimeToUTC end)
 where
  start = zt { zonedTimeToLocalTime = startOfMonth (zonedTimeToLocalTime zt) }
