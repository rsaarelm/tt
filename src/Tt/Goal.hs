...
dataPoints goal db =
    sortBy (\a b -> dataPointDay a `compare` dataPointDay b) points
  where
    points = filter (belongsIn goal) $ mapMaybe toDataPoint db
...
progressStats db day goal = (current, progressDay - absoluteDay)
  where
    ...
    progressDay = if end /= start then
      nDays * (current - start) / (end - start)
      else 0
